    ...
    if (fs_1.default.lstatSync(configDir).isDirectory()) {
        ...
        if (main) {
            ...
            output.requireContexts = output.stories.map((specifier) => {
                ...
                return global_1.global.__requireContext(workingDir, basePath, recursive, match);
            });
        }
        return output;
    }
...
function configure(options) {
    ...
    global_1.global.FEATURES = features;
    global_1.global.STORIES = stories.map((specifier) => ({
        ...specifier,
